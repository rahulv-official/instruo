  ...
  <div class="flex w-full flex-col flex-wrap gap-4 py-8">
    ...
    >
      <UiAccordionItem
        ...
        :key="`categoryCategory _${category.name + index}`"
        :value="category.category"
      >
        <UiAccordionTrigger>
          <span>{{ index + 1 + ". " + category.name }} Tools</span>
        </UiAccordionTrigger>
        <UiAccordionContent>
          <ClientOnly>
            <CardSpotlight
              v-for="(tool, indexJ) in category.tools"
              :key="`toolIndex_${tool.title + indexJ}`"
            ...
            >
              <NuxtLink :to="tool.path">
                <div class="flex w-full flex-row items-center justify-center">
                  ...
                  <div class="mr-2 font-heading">{{ tool.title }}</div>
                  <Badge
                    v-for="badge in tool.navBadges"
                    :key="`${badge}_${tool.title}_${index}`"
                    :type="badge.type"
                  >
                    {{ badge.value }}
                  </Badge>
